...
namespace BINDU
{
	...
	public:
		...
		ComPtr<ID3D12Resource>	CreateCommitedTex2D(std::uint64_t width, std::uint32_t height, DXGI_FORMAT format, D3D12_HEAP_FLAGS heapFlag, const D3D12_CLEAR_VALUE* optimizedClearVal = nullptr) const;
		...
		[[nodiscard]] DirectXConstantBuffer&&		CreateConstantBuffer(std::uint32_t elementCount) const;
