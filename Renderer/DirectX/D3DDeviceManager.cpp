...
namespace BINDU
{
	...
	ComPtr<ID3D12Resource> D3DDeviceManager::CreateCommitedTex2D(std::uint64_t width, std::uint32_t height,
		DXGI_FORMAT format, D3D12_RESOURCE_FLAGS resourceFlags, D3D12_HEAP_FLAGS heapFlag, const D3D12_CLEAR_VALUE* optimizedClearVal) const
	{
		...
		auto rsrcDesc = CD3DX12_RESOURCE_DESC::Tex2D(format, width, height, 1, 1, 1, 0, resourceFlags);
	...
	ComPtr<ID3D12Resource> D3DDeviceManager::CreateDefaultBuffer(const void* initData, UINT64 byteSize,
		ID3D12GraphicsCommandList* commandList, ComPtr<ID3D12Resource>& uploadBuffer) const
	{
		return D3DUtility::CreateDefaultBuffer(m_d3dDevice.Get(), commandList, initData, byteSize, uploadBuffer);
	}
	...
	Ref<DirectXConstantBuffer> D3DDeviceManager::CreateConstantBuffer(std::uint32_t elementCount, std::uint32_t elementByteSize) const
	{
		auto buffer = CreateRef<DirectXConstantBuffer>(elementCount, elementByteSize);
		...
		buffer->m_resource->Map(0, nullptr, reinterpret_cast<void**>(&buffer->m_mappedData));
